...
<%User user = (User)session.getAttribute("user");
  ...
  String sqljobno = "select job_no,allocation_date from joballocation where job_no ='"+job_no+"'";
...
<div class="container-fluid">
  <div class="row">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
      ...
      <form method="get" action="updatejobwork_process_submit.jsp">
        <div class="form-row">
          <div class="form-group col-md-4">
            Job No
            <select class="custom-select" name="job_no">
              <option selected></option>
              <%
              while(rsjobno.next()){%>
              <option value="<%=rsjobno.getString("job_no")%>"
                  <%if(rsjobno.getString("job_no").equals(job_no))
                     { allocation_date = rsjobno.getString("allocation_date");
                      %>
                        selected
                     <%}
                    %>
                ><%=rsjobno.getString("job_no")%></option>
            <%}
              %>
      </select>
          </div>
          <div class="form-group col-md-4">
            ...
            <%
              //Statement 
            %>
            <input type="date" class="form-control" id="startDate" name="startDate" readonly value="<%=allocation_date%>">
          </div>
          <div class="form-group col-md-4">
            Complete Date
